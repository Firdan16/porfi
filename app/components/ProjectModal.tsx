        ...
        <div ref={overlayRef} className="fixed inset-0 z-[100] flex items-end justify-center bg-[var(--navy)]/80 p-0 sm:items-center sm:p-4 lg:p-8" onClick={(event) => event.target === event.currentTarget && handleClose()} onKeyDown={handleKeyDown}>
            <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby={`project-title-${project.id}`} aria-describedby={`project-description-${project.id}`} className="relative flex h-[100dvh] max-h-[100dvh] w-full max-w-[1180px] flex-col overflow-y-auto overscroll-contain bg-[var(--paper)] sm:h-[min(92dvh,900px)] sm:max-h-none sm:overflow-y-auto lg:grid lg:grid-cols-[minmax(0,0.92fr)_minmax(0,1.08fr)] lg:overflow-hidden">
                ...
                <div className="order-1 min-h-max shrink-0 border-b border-[var(--line)] p-5 pb-10 sm:p-8 lg:order-1 lg:min-h-0 lg:shrink lg:overflow-y-auto lg:border-b-0 lg:border-r lg:p-10">
                    <div className="flex items-start justify-between gap-4 pr-12 sm:pr-0"><p className="archive-caption text-[var(--identity)]">{project.category}</p><span className="flex items-center gap-3"><span className="hidden border border-[var(--line)] px-2 py-1 sm:inline-flex"><StoreMarks project={project} iconClassName="h-3.5 w-3.5 shrink-0 text-[var(--identity)]" /></span><span className="archive-caption">{project.version}</span></span></div>
                    ...
                    {hasStoreListing(project) && <div className="mt-7 border border-[var(--ink)] bg-[var(--paper-light)] p-4"><p className="archive-caption mb-3 flex items-center gap-2"><span className="h-1.5 w-1.5 shrink-0 bg-[var(--identity)]" aria-hidden="true" />{t("availableOn")}</p><StoreButtons project={project} /></div>}
                ...
                {project.kind === "package" && project.demoVideo ? (
                    ...
                    <div className="order-2 flex min-h-[clamp(300px,48dvh,560px)] flex-col bg-[var(--navy)] p-3 sm:p-6 lg:order-2 lg:min-h-0 lg:p-8" style={{ backgroundColor: project.color }}>
                        <div className="relative min-h-0 flex-1 overflow-hidden" onPointerDown={handlePointerDown} onPointerUp={handlePointerUp}>
                            {(project.images ?? []).map((image, index) => <div key={image} className={`absolute inset-0 flex items-center justify-center px-[8%] py-8 sm:px-[12%] sm:py-10 ${activeIdx === index ? (slideDirection === "next" ? "project-slide-enter-next" : "project-slide-enter-prev") : "pointer-events-none opacity-0"}`} aria-hidden={activeIdx !== index}><Image src={image} alt={t("slideAlt", { title: project.title, index: index + 1 })} fill sizes="(min-width: 1024px) 380px, 180px" priority={activeIdx === index} className="object-contain" /></div>)}
                        ...
                        {hasStoreListing(project) && <div className="flex flex-wrap items-center justify-center gap-x-4 gap-y-2 pt-4"><span className="archive-caption text-white/60">{t("availableOn")}</span><StoreMarks project={project} showLabels iconClassName="h-4 w-4 shrink-0" labelClassName="text-[11px] font-bold uppercase tracking-[0.12em]" className="text-white" /></div>}
                    </div>
                )}
            </div>
        </div>,
